...
function _tipShow(key, x, y) {
  ...
  for (const [sec, rows] of Object.entries(sections)) {
    ...
    rows.forEach(({k,v}) => {
      const short = v.length > 65 ? v.slice(0,63)+'…' : v;
      html += `<div style="display:flex;gap:8px;margin-bottom:3px"><span style="color:rgba(255,255,255,.5);min-width:100px;flex-shrink:0;font-size:11.5px">${k}</span><span style="font-weight:500">${short}</span></div>`;
    });
  }
  _tip.innerHTML = html;
  _tipMove(x, y);
...
function _tipMove(x, y) {
  const left = (x + 16 + 320 > window.innerWidth) ? x - 328 : x + 16;
  const top  = Math.max(8, Math.min(y - 20, window.innerHeight - 320));
  _tip.style.left = left + 'px';
  _tip.style.top  = top + 'px';
}

function _tipHide() { _tip.style.opacity = '0'; }

// Build custom dropdown options list
function buildScannerDropdown(vals) {
  console.log('🔨 buildScannerDropdown called with:', vals);
  const list = document.getElementById('scanner-options');
  if (!list) { console.error('❌ scanner-options element missing'); return; }
  list.innerHTML = '';
  ...
  vals.forEach(val => {
    ...
    item.onclick = () => _selectScanner(val, val);
    item.addEventListener('mouseenter', e => _tipShow(val, e.clientX, e.clientY));
    item.addEventListener('mousemove',  e => _tipMove(e.clientX, e.clientY));
    item.addEventListener('mouseleave', _tipHide);
...
function toggleScannerDropdown() {
  const list = document.getElementById('scanner-options');
  if (!list) { console.error('❌ scanner-options not found'); return; }
  const isOpen = list.style.display !== 'none';
  if (isOpen) { list.style.display = 'none'; return; }
  console.log('🔍 SKU_DB length:', typeof SKU_DB !== 'undefined' ? SKU_DB.length : 'undefined');
  ...
  if (list.children.length === 0 && typeof SKU_DB !== 'undefined' && SKU_DB.length) {
    ...
    console.log('🔍 scanner vals:', vals);
    buildScannerDropdown(vals);
    console.log('🔍 after build, children:', list.children.length);
  }
