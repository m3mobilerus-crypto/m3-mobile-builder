...
_overlay.style.cssText = 'position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,.45);opacity:0;transition:opacity .15s;pointer-events:none';
  ...
  for (const [sec, rows] of Object.entries(sections)) {
    ...
    rows.forEach(({k,v}) => {
      bodyHTML += `<div style="display:flex;gap:10px;margin-bottom:5px">
        <span style="color:rgba(255,255,255,.5);min-width:110px;flex-shrink:0;font-size:11.5px;padding-top:1px">${k}</span>
        <span style="font-weight:500;color:#fff">${v}</span>
      </div>`;
    });
  }

  _tip.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px 12px;border-bottom:1px solid rgba(255,255,255,.1);flex-shrink:0">
      <span style="font-size:15px;font-weight:700;color:#60A5FA">${key}</span>
      <button onclick="_tipHide()" style="background:rgba(255,255,255,.1);border:none;color:#fff;width:26px;height:26px;border-radius:50%;cursor:pointer;font-size:14px;line-height:1;display:flex;align-items:center;justify-content:center">✕</button>
    </div>
    <div style="padding:4px 16px 16px;overflow-y:auto;flex:1;-webkit-overflow-scrolling:touch">${bodyHTML}</div>
  `;
...
function _tipMove(x, y) { /* no-op — centered */ }
...
_overlay.addEventListener('click', _tipHide);
